...
The values in the *jvmArgs* list may contain the placeholder `{{BIN_DIR}}`, which stands for the _bin_ directory of your custom runtime image.
...
NOTE: If the default value is used, and it contains the placeholder `{{BIN_DIR}}`, the plugin replaces this placeholder with `$APPDIR/..` when passing the arguments to `jpackage`.
